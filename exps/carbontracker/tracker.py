...
class CarbonTrackerThread(Thread):
    ...
    def total_energy_per_epoch(self):
        ...
        energy_divided = []
        print("comps", self.components, len(self.components))
        for comp in self.components:
            #print(comp)
            energy_usage = comp.energy_usage(self.epoch_times)
            print("energy", comp.name, energy_usage)
            energy_divided.append(np.copy(np.array(energy_usage)))
            #print("energy_divided",energy_divided)
            for i,en in enumerate(energy_usage):
                energy_usage[i] = energy_usage[i].sum()
            total_energy += energy_usage
            #print("energy_total", comp.name, total_energy)
        return energy_divided, total_energy * constants.PUE_2022
...
class CarbonTracker:
    ...
    def epoch_end(self):
        ...
        try:
            ...
            if self.epoch_counter == self.monitor_epochs:
                timing, energy, divided = self._output_actual()
            ...
            for element in divided:
                for new_element in element[0]:
                    #print(new_element)
                    new_divided.append(new_element)
            print(new_divided)

            #print(np.array(divided), "flat", np.array(divided).flatten())
            return timing, energy, np.array(new_divided)*constants.PUE_2022
        except Exception as e:
            self._handle_error(e)
    ...
    def stop(self):
        ...
        self.epoch_counter -= 1
        timing, energy, _ = self._output_actual()
        print("AQUI", timing,energy)
    ...
    def _output_energy(self, description, time, energy, co2eq=None, conversions=None):
    ...
    def _output_actual(self):
        ...
        divided, energy_usages = self.tracker.total_energy_per_epoch()
        ...
        if self.epochs_before_pred == 0:
            self._output_energy("Actual consumption:", time, energy)
        else:
            self._output_energy(f"Actual consumption for {self.epoch_counter} epoch(s):", time, energy)
        return time, energy, divided
